...
void VCOAdj0(VCO *vco) {
    ...
    while(1) {
        ...
        if(semaphoreid == 0) vco->Getdac()->Clradjusted();
...
void VCO::Info(void) {
    ...
    for (int octave=0; octave<(m_octaves+1); octave++) printf("%d ", m_octavemuls[octave]);
    printf("]\n\r");
}

void VCO::StartAdj(void) {
    if(m_vcoadjfunc) m_thread.start(callback(m_vcoadjfunc, this));
}
...
OctaveXstep VCO::Tritone(int8_t tritone) { // returns octave and xstep
    OctaveXstep x;
...
uint32_t VCO::Getfreq16(int16_t octave, int16_t xstep) { // returns the frequency in Hz * 16
    //printf("G16: %d %d\n\r", octave, xstep);
...
uint16_t VCO::Getfreq(int16_t octave, int16_t xstep) { // returns the frequency in Hz
    m_omul = m_octavemuls[octave];
    m_hmul = XSTEPMULS[xstep];
    m_freq = m_omul*m_hmul;
    return(m_freq>>(FPWR));
...
WidthFreq VCO::Getwidth(int16_t octave, int16_t xstep) { // returns wave width
    WidthFreq w;
...
int8_t VCO::Getoctave(uint32_t din) { // return the octave which corresponds to the DAC's digital input value
    return(din/m_dins_per_volt);
}

int16_t VCO::Getstep(uint32_t din) {  // return the xstep which corresponds to the DAC's digital input value;
    m_dinmod = (din%m_dins_per_volt)<<XPWR;
    return(m_dinmod/m_dins_per_halfstep);
}

int16_t VCO::Getoffset(int8_t tritone) {         // returns the offset for the tritone
...
int16_t VCO::Getdinoffset(uint32_t din) {        // returns the amount of offset for the DAC's digital input value
...
WidthFreq VCO::Dintowidth(uint32_t din) { // returns width, tfreq16
    return(Getwidth(Getoctave(din), Getstep(din)));
}

void VCO::Setoffset(int8_t tritone, int16_t offset) { // set the offset for the tritone
    m_offsets[tritone] = offset;
}

uint32_t VCO::Dinh(int8_t octave, int8_t halfstep) { // return DAC's digital input value
    return(octave*m_dins_per_volt + halfstep*m_dins_per_halfstep);
}

uint32_t VCO::Dinx(int8_t octave, int16_t xstep) { // return DAC's digital input value
    return((uint32_t)octave*(uint32_t)m_dins_per_volt + (((uint32_t)xstep*(uint32_t)m_dins_per_halfstep)>>XPWR));
}

uint32_t VCO::Dinj(int8_t octave, float ratio) { // return DAC's digital input value ... a ratio of 3/2 is a fifth above the octave in just intonation
    return(log2((float)pow(2, octave)*ratio)*(float)m_dins_per_volt + .5);
}

uint32_t VCO::Dinf(uint32_t freq16) {       // return DAC's input value for frequency*16
    return(log2((float)freq16/(m_lowfreq*16.0))*(float)m_dins_per_volt + .5);
}

float VCO::Getvolts(uint32_t din) {         // return the voltage which corresponds to the DAC's digital input value
    return(m_lowvolts + ((float)din/(float)m_dins_per_volt));
}

OctaveXstep VCO::Freqtooctave(uint32_t freq16) {    // return octave and xstep for frequency*16
    m_din = Dinf(freq16);
    OctaveXstep x;
...
void VCO::Tuneup(int8_t tritone, float sleep, bool printit) { // tune to the tritone frequency
   ...
   int32_t freqdin;
   bool firsttime = true;
   if (printit) printf("tritone %d, targetdin %6d, targetfreq %7.1f, oct %d,%d\n\r",
   ...
   float total = 0.0;
   for (i=0; i<=cnt; i++) total += (float)offsets[i];
   offset = round(total/(float)(cnt+1));
   Setoffset(tritone, offset);
   if (printit) printf("%2d %2d %5d %7.1f %s", tritone, cnt, offset, freq16/16.0, Getoffsets());
}

void VCO::Tuneups(float sleep, bool printit) { // tune to the tritone frequency
    m_dac->Reset();
    ...
    for (tritone=16; tritone < 20; tritone++) {
        ...
        if (offset1 == 0 and offset0 !=0) Setoffset(tritone+1, offset0);
    }
